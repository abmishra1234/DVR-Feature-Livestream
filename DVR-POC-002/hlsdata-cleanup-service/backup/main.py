...
def setup_logging():
    ...
    handler = TimedRotatingFileHandler(logfile_name, when="midnight", interval=1)
    handler.suffix = "%Y-%m-%d.log"
    handler.extMatch = r"^\d{4}-\d{2}-\d{2}.log$"

    logging.basicConfig(filename=logfile_path, level=logging.DEBUG, format=log_format)
    logging.info("Logging is set up.\n\n\n")

def clean_old_segments(directory, retention_period, exception_set):
    """Deletes files older than the retention period from the specified directory, except for those in the exception set."""
