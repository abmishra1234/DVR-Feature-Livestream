    ...
    try:
        while time.time() < end_time or end_time == -1:
            for resolution, playlist_url in playlists:
                ...
                playlist_path = download_dir / resolution / 'playlist.m3u8'
            ...
            for language, subtitle_url in subtitles:
                subtitle_playlist_path = download_dir / language / 'subtitles.m3u8'
        ...
        logging.error(f"Failed to download playlist or segments: {e}")
...
def main():
    ...
    playlists, subtitles, closed_captions = parse_master_manifest(HLS_URL, DOWNLOAD_DIR)

    with concurrent.futures.ThreadPoolExecutor(max_workers=THREAD_COUNT) as executor:
        futures = [
            executor.submit(download_and_update_manifest, playlists, subtitles, end_time, DOWNLOAD_DIR, THREAD_COUNT, SEGMENT_TIMEOUT, SLEEP_INTERVAL)
        ]
